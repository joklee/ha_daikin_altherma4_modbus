...
@pytest.mark.asyncio
async def test_config_flow_user_step_creates_entry_successfully(monkeypatch):
    """Test successful config flow user step creating an entry."""
...
@pytest.mark.asyncio
async def test_config_flow_user_step_validates_invalid_host(monkeypatch):
    """Test that config flow rejects invalid host."""
    config_flow_module = _load_config_flow_module(monkeypatch)
    flow = config_flow_module.ConfigFlow()
...
@pytest.mark.asyncio
async def test_config_flow_user_step_validates_invalid_port(monkeypatch):
    """Test that config flow rejects invalid port."""
    config_flow_module = _load_config_flow_module(monkeypatch)
    flow = config_flow_module.ConfigFlow()
...
@pytest.mark.asyncio
async def test_config_flow_user_step_validates_scan_intervals(monkeypatch):
    """Test that config flow validates scan interval relationships."""
    config_flow_module = _load_config_flow_module(monkeypatch)
    flow = config_flow_module.ConfigFlow()
...
async def test_config_flow_get_options_flow_returns_options_flow_instance(monkeypatch):
    ...
    options_flow = config_flow_module.ConfigFlow.async_get_options_flow(config_entry)
...
async def test_config_flow_user_step_separates_data_and_options(monkeypatch):
    """Test that config flow properly separates data and options."""
    config_flow_module = _load_config_flow_module(monkeypatch)
    flow = config_flow_module.ConfigFlow()
...
@pytest.mark.asyncio
async def test_options_flow_successful_update(monkeypatch):
    """Test successful options flow configuration update."""
    config_flow_module = _load_config_flow_module(monkeypatch)
...
@pytest.mark.asyncio
async def test_options_flow_optional_electric_power_sensor(monkeypatch):
    """Test that electric_power_sensor is optional in options flow."""
    config_flow_module = _load_config_flow_module(monkeypatch)
    ...
    # Test with empty electric_power_sensor
    user_input = {
        "scan_interval": 20,
        "slow_scan_interval": 400,
        "demo_mode": True,
    ...
    # Verify electric_power_sensor is not included when empty
    options_data = result["data"]
    assert "electric_power_sensor" not in options_data

    # Test with electric_power_sensor provided
    user_input = {
        "scan_interval": 20,
        "slow_scan_interval": 400,
        "demo_mode": True,
        "electric_power_sensor": "power_meter",
    }
    ...
    # Verify electric_power_sensor is included when provided
    options_data = result["data"]
    assert options_data["electric_power_sensor"] == "power_meter"
