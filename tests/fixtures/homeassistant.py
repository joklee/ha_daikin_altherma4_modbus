"""Home Assistant mocking stubs and fixtures for tests.

This module is the single source of truth for Home Assistant mocking.

``install_homeassistant_stubs`` installs lightweight ``homeassistant.*``
fake modules into ``sys.modules`` so test modules (and the custom
component modules they import) can import from ``homeassistant`` without
a real Home Assistant installation. It must run before any test imports
``homeassistant``; ``tests/conftest.py`` imports this module at collection
time which triggers installation.

Entity-level mocking uses ``unittest.mock.Mock`` / ``SimpleNamespace``.
"""
...
def install_homeassistant_stubs() -> None:
    """Install Home Assistant stub modules into ``sys.modules``.

    Must be called before any test imports ``homeassistant``. Re-running
    overwrites the same stub names with the same fake modules.
    """
    ...
    # Create a global homeassistant.helpers module
    ...
    class _HADataUpdateCoordinator:
        ...
        def async_add_listener(self, callback, context=None):
            """Mock listener registration returning an unsubscribe callable."""
            return lambda: None
    ...
    class _HomeAssistantError(Exception):
        """Mock HomeAssistantError that accepts keyword arguments."""
    ...
    # IoT-class marker consumed by the modbus mock client on import.
    ...
    # must register themselves via the ``domain=`` class keyword, exactly like
    # the real homeassistant.config_entries.ConfigFlow does. Without this,
    # Home Assistant cannot resolve the flow handler for stored config entries.
...
# Install stubs as soon as this module is imported (conftest triggers it).
_install_stubs_once()
