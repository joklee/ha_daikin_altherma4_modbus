...
def _load_modbus_client_module(monkeypatch, mock_pymodbus=True):
    ...
    module_name = f"{package_name}.modbus.modbus_client"
    ...
    exceptions_module.DaikinModbusException = DaikinModbusException
    exceptions_module.ModbusConnectionException = ModbusConnectionException
    ...
    exceptions_module.ModbusInvalidAddressException = ModbusInvalidAddressException
    monkeypatch.setitem(sys.modules, exceptions_name, exceptions_module)

    # Install a fake modbus parent module so the real modbus/__init__.py
    # (which pulls in many dependencies) is not executed
    base_path = (
        Path(__file__).resolve().parents[2]
        / "custom_components"
        / "ha_daikin_altherma4_modbus"
    )
    modbus_parent_name = f"{package_name}.modbus"
    modbus_parent = types.ModuleType(modbus_parent_name)
    modbus_parent.__path__ = [str(base_path / "modbus")]
    monkeypatch.setitem(sys.modules, modbus_parent_name, modbus_parent)

    # Install a fake core parent and core.const so the real core/__init__.py
    # (which pulls in data_manager etc.) is not executed
    core_parent_name = f"{package_name}.core"
    core_parent = types.ModuleType(core_parent_name)
    core_parent.__path__ = [str(base_path / "core")]
    monkeypatch.setitem(sys.modules, core_parent_name, core_parent)

    core_const_name = f"{package_name}.core.const"
    core_const_module = types.ModuleType(core_const_name)
    core_const_module.SPECIAL_REGISTER_VALUES = frozenset({32765, 32766, 32767})
