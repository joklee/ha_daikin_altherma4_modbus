...
def _load_integration_module(monkeypatch):
    ...
    coordinator_name = f"{package_name}.integration.coordinator"
    ...
    # Install fake parent packages so real subpackage __init__.py files
    # are not executed (they would override our mocks)
    base_path = (
        Path(__file__).resolve().parents[2]
        / "custom_components"
        / "ha_daikin_altherma4_modbus"
    )
    for sub in ("integration", "modbus"):
